...
> **정정 (2026-07-11, `03_features.ipynb` §8-B에서 확인)**
>
> 위 문장 중 "GFS 850hPa를 **허브 조건으로** 쓰라"는 표현은 과했다. 두 가지를 혼동했다.
>
> 1. **GFS 자체의 변수들끼리 비교**하면 850hPa가 2m보다 허브에 가깝다 — 이 부분은 맞다.
> 2. 그러나 **LDAPS는 태백산맥을 제대로 잡는다.** 격자 지형고도(`surface_0_h`)가 868~1,001m로
>    실제 단지 고도와 일치한다(§2-3). 즉 **LDAPS의 2m 기온·습도는 이미 허브 근처 고도의 값**이다.
>
> 따라서 GFS 850hPa의 가치는 LDAPS 2m의 **"대체재"가 아니라, 다른 예보 모델에서 온 독립적인 보조 추정치**다.
> 실제로 재 보니 `gfs_850_t`는 `ldaps_2m_t`와 상관 0.967, `gfs_850_r`는 `ldaps_2m_r`와 상관 0.62였다.
> 기대는 낮추고 추가만 했으며, 효과 측정 결과는 `reports/phase4b_features_group3.md`에 있다.
>
> **덤으로 발견한 것**: GFS의 복사량(`dswrf`)은 **예보 시작 이후의 평균 플럭스**다(21시에 115 W/m²).
> 반면 LDAPS `NDNSW`는 **순간값**이다(21시에 정확히 0). 안정도 피처에는 LDAPS를 써야 한다.
