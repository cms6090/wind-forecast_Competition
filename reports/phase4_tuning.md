...
2025년은 물리 예측치 기준 이용률이 2024년보다 뚜렷이 높다.
...
> **정정 (2026-07-11)** — 처음에 나는 "바람이 세면 이용률이 정격 근처에 몰려 파워커브의 평평한 구간에
> 들어가므로 예측이 쉬워진다. 따라서 **2025년은 더 쉬운 해**"라고 적었다. **데이터로 확인해 보니 틀렸다.**
>
> 물리 예측치(`pc_pred`)로 연도별 난이도를 재 봤다.
>
> | 연도 | `pc_pred` total_score | 실제 평균 이용률 |
> |---|---:|---:|
> | 2023 | 0.5531 | 0.3091 |
> | **2024** | **0.5718** | **0.2845** |
>
> **이용률이 더 높은 2023년의 점수가 오히려 낮다.** "높은 이용률 = 쉬운 해"라는 관계가 성립하지 않는다.
> 게다가 2023년은 파워커브를 그 기간 SCADA로 학습했으므로 **점수가 낙관적인데도** 2024보다 낮다.
> 실제 격차는 더 크다는 뜻이다.
>
> **바로잡은 결론**: 2025년이 쉬운 해라는 근거는 없다. 다만 **연도 간 변동 폭이 최소 0.019**(2023↔2024)이며,
> 이는 1등과의 격차(0.036)의 절반이나 된다. 로컬 점수와 리더보드 점수를 직접 빼서 비교하면 안 된다.
>
> **첫 제출 후 Public 점수와 로컬 2024 점수의 차이를 실험 로그에 기록해 그 오프셋을 실측**해야 한다
> (CLAUDE.md 5번). 그 값을 알기 전까지는 어느 쪽 방향으로도 단정하지 않는다.

### 4-6. 1등과의 격차 분해 (2026-07-11, 1등 점수 공개 후)

1등: **1−NMAE 0.87932 / FICR 0.45473 / total 0.66703** (2025 Public).
우리 Phase 4: 1−NMAE 0.86355 / FICR 0.39795 / total 0.63075 (2024 홀드아웃).

**주의**: 서로 다른 해를 본 숫자이므로 직접 비교는 불가능하다(위 정정 참조). 그럼에도 **격차의 성분비**는 알아낼 수 있다.

우리 모델의 **오차 분포 모양은 그대로 두고 크기만 균일하게 축소**해, 1등과 같은 NMAE에 도달시켜 봤다.

| 오차 배율 | NMAE | 1−NMAE | FICR | total |
|---:|---:|---:|---:|---:|
| 1.000 (현재) | 0.13645 | 0.86355 | 0.39795 | 0.63075 |
| **0.8845** | **0.12068** | **0.87925** | **0.44295** | **0.66105** |
| 0.8592 | 0.11723 | 0.88277 | **0.45473** | 0.66875 |

**우리 오차를 1등과 같은 크기로 줄이면 FICR이 0.443까지 저절로 따라온다.** 1등은 0.4547이므로 차이는 +0.0118뿐이다.

총 격차 0.03628의 성분:

| 성분 | 크기 | 비중 |
|---|---:|---:|
| **정확도 (NMAE)** | +0.03030 | **83.5%** |
| 밴드 기술 (같은 NMAE에서의 FICR 우위) | +0.00589 | 16.2% |

**해석**: 우리의 산식-인지 작업(τ 상향, `actual` 가중)은 **밴드에서 얻을 것의 대부분을 이미 얻었다.**
남은 격차는 대부분 **순수한 예측 정확도**다.

**구체적인 목표**: 현재 오차 모양을 유지한 채 **오차를 14.1% 줄이면**(×0.8592) FICR 0.4547에 도달하고,
그때 1−NMAE는 0.88277로 1등(0.87932)보다 높으며 total은 **0.66875**가 된다.

**단, 1등 점수의 불확실성도 감안해야 한다.** Public은 평가 데이터의 40% 표본이고(그룹당 채점 시간 약 2,450개,
부트스트랩 표준오차 ±0.005 수준), 1등 점수는 **여러 제출 중 최고점**이라 승자의 저주로 부풀려져 있다.

> **정정 (2026-07-11, exp016 제출 후 Public 점수 확인)**
>
> 위 §4-6의 격차 분해("83.5%가 정확도")는 **2024년 오차 분포로 만든 반사실**이었다.
> 실제로 제출해 **같은 데이터(2025 Public) 위의 숫자**를 얻고 보니 **결론이 뒤집힌다.**
> 자세한 재분석은 `reports/phase5_final.md` §5에 있다. 요약하면:
>
...
>
> **같은 데이터에서 재면 FICR 격차가 66%다.** 반사실 계산이 틀린 이유는
> "오차를 균일하게 축소하면 FICR이 이렇게 오른다"는 관계가 **연도를 넘어 성립하지 않기 때문**이다
> (우리 2024는 NMAE 0.1364에 FICR 0.3980, 2025는 NMAE 0.1507에 FICR 0.3964 — NMAE가 훨씬 나쁜데 FICR은 같다).
>
