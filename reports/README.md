...
- [phase6_nn_metric_loss.md](phase6_nn_metric_loss.md) — `src/nn.py` + `train.ipynb` + `inference.ipynb`: **대회 산식(FICR 계단)을 시그모이드로 근사해 손실함수로 직접 최적화하는 MLP**. LightGBM 커스텀 목적함수가 왜 실패하는지 규명(잎 값 보정 부재), MLP+LightGBM 블렌드로 **2024 홀드아웃 0.6308 → 0.6458 (+0.0151), 1−NMAE와 FICR 동시 개선**. 앞선 12개 가설 기각의 공통 원인(예보 파생 피처는 트리에 정보 0) 정리
